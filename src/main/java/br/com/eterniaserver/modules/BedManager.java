    ...
    public BedManager(EterniaServer plugin) {
        if (EterniaServer.configs.getBoolean("modules.bed")) {
            if (EterniaServer.configs.getBoolean("server.async-check")) {
                plugin.getServer().getScheduler().runTaskTimerAsynchronously(plugin, new BedTimer(plugin), 0L, EterniaServer.configs.getInt("server.checks") * 20);
            } else {
                plugin.getServer().getScheduler().runTaskTimer(plugin, new BedTimer(plugin), 0L, EterniaServer.configs.getInt("server.checks") * 20);
            }
