...
public class TableCfg {
    ...
    public TableCfg() {
        ...
        if (EterniaLib.getMySQL()) {
            createTable = new CreateTable(EterniaServer.getString(ConfigStrings.TABLE_KITS));
            createTable.columns.set("id INT AUTO_INCREMENT NOT NULL PRIMARY KEY", "name VARCHAR(32)", "cooldown BIGINT(20)");
            SQL.execute(createTable);

            createTable = new CreateTable(EterniaServer.getString(ConfigStrings.TABLE_PLAYER));
            ...
            createTable = new CreateTable(EterniaServer.getString(ConfigStrings.TABLE_LOCATIONS));
            createTable.columns.set("id INT AUTO_INCREMENT NOT NULL PRIMARY KEY", "name VARCHAR(32)", "location VARCHAR(64)");
            SQL.execute(createTable);

            createTable = new CreateTable(EterniaServer.getString(ConfigStrings.TABLE_REWARD));
