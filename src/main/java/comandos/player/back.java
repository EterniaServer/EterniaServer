...
public class back implements CommandExecutor {
    private final Economy economy = principal.main.getEconomy();
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            Player player = (Player) sender;
            if (principal.playerlistener.back.containsKey(player)) {
                if(player.hasPermission("eternia.backfree")) {
                    player.teleport(principal.playerlistener.back.get(player));
                    principal.playerlistener.back.remove(player);
